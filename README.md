...
OpenNDS（OpenWrt 上で動作する Captive Portal）の見た目をカスタマイズするテーマと、認証コンテナと連携して合言葉や LDAP 認証を行うためのサンプル（CGI スクリプト／設定例）を含める予定です。
...
- 認証フローを以下のいずれかに切り替えられるようにする
  ...
  - 合言葉による認証
  - ユーザー名／パスワード（LDAP）による認証
- 認証処理自体は OpenWrt 上の OpenNDS ではなく外部の CGI 実行可能なサーバーで行えるようにする
...
- 認証モードの切り替え方法
  - docker/data/.apmodeファイルに書き込み
    - 0:ボタンを押すだけで認可 / 1: 合言葉 / 2: LDAP
    - 合言葉はdocker/data/.appassファイルに書き込み

- LDAP認証を行う場合は docker/data/web/api/v1/opennds/ldap.cgiのLDAPホストを書き換える
